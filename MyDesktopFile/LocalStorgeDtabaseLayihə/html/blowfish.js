...
var Base64 = {
    ...
    encode : function (plain) {
        ...
        plain = Base64._utf8_encode(plain);
    
        while (i < plain.length) {
    
            chr1 = plain.charCodeAt(i++);
            chr2 = plain.charCodeAt(i++);
            chr3 = plain.charCodeAt(i++);
    ...
    decode : function (b64data) {
        ...
        while (i < b64data.length) {
            ...
            enc1 = Base64._keyStr.indexOf(b64data.charAt(i++));
            enc2 = Base64._keyStr.indexOf(b64data.charAt(i++));
            enc3 = Base64._keyStr.indexOf(b64data.charAt(i++));
            enc4 = Base64._keyStr.indexOf(b64data.charAt(i++));
    ...
    // main functions specific to this web page:
    function encode(obj) {
            var plaintxt = obj.txt.value;
        var oneline;
        var formatted_output="";
        var i = 0;
        var bf;
        obj.err.value="";
        if (obj.key.value.length <8 || obj.key.value.length > 56){
            obj.err.value="ERROR: key must be 8 to 56 characters long";
            return;
        }
        obj.err.value="OK: wait, encrypting...";
        bf= new Blowfish(obj.key.value);
        // plaintext ve ciper text block size qeder olmalidi
        // blowfish 8 baytliq block cipher di
        for (var f = 0; f < plaintxt.length % 8; f ++) plaintxt+= "\t";
		//microsoft ms-ie btoa desteylemir ona gore Base64.encode isletdik:
            //oneline = window.btoa(bf.encrypt(plaintxt));
            oneline = Base64.encode(bf.encrypt(plaintxt));
        while (i < oneline.length){
            formatted_output = formatted_output + oneline.charAt(i);
            i++;
            // a line len of 64 will make this more resisant against
            // missing lines. base64 encodes 3 characters into 4
            // and blowflish works on units of 8 characters.
            if (i % 64==0){
                formatted_output = formatted_output + "\n";
            }
        }
        obj.err.value="";
            obj.txt.value = formatted_output;
    }
    function decode(obj) {
            var cipher_b64 = obj.txt.value;
            var cipher;
            var plaintext;
            var sanitycheck="";
        var bf;
        ...
        sanitycheck=cipher_b64.substr(2,40);
        if (sanitycheck.match(/[ \t]/)){
            obj.err.value="ERROR: not a cipher text, can not decrypt";
            return;
        }
        obj.err.value="OK: wait, decrypting...";
        bf= new Blowfish(obj.key.value);
        cipher_b64=cipher_b64.replace(/[^A-Za-z0-9\+\/\=]/g,"");
            // microsoft MS-IE does not support atob, we use Base64.decode:
        //cipher=window.atob(cipher_b64);
        cipher=Base64.decode(cipher_b64);
            plaintext = bf.decrypt(cipher);
        plaintext = plaintext.replace(/\t+$/,""); // remove padding if any
        obj.err.value="";
            obj.txt.value = plaintext;
    }